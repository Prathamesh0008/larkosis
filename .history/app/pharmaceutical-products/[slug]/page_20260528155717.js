...
export default async function PharmaceuticalProductDetailPage({ params }) {
  ...
  <div className="mb-5 rounded-3xl border border-[#ead8cf] bg-[#fffdfb] p-3 shadow-sm">
    <div className="flex gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible">
      {contentSections.map((section, index) => (
        <a
          key={section.id}
          href={`#${section.id}`}
          className="group inline-flex shrink-0 items-center gap-2 rounded-full border border-[#f0d9cb] bg-white px-4 py-2 text-xs font-bold text-[#0f3558] shadow-sm transition-all hover:border-[#ec671f] hover:bg-[#fff3eb] hover:text-[#ec671f]"
        >
          <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#ec671f]/10 text-[10px] font-black text-[#ec671f] group-hover:bg-[#ec671f] group-hover:text-white">
