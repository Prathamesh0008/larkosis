...
export default function Navbar({ companyProfile }) {
  ...
  const mobileSubItemClass = (isActive) =>
    `cursor-pointer rounded-lg px-3 py-2 transition-all duration-200 ${isActive
  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-50 w-full overflow-x-clip bg-white/95 shadow-md backdrop-blur-md">
      <div className="mx-auto flex h-[72px] w-full max-w-7xl min-w-0 items-center justify-between px-4 sm:h-[95px] sm:px-6 lg:h-[96px] xl:px-10 2xl:px-0">
        <Link href="/" className="flex min-w-0 shrink-0 items-center max-[360px]:max-w-[72%]">
          <Image
            src="/Larksois_Pharma_Cropped.png"
            alt="Larkosis Pharma Logo"
            width={240}
            height={80}
            priority
            className="mt-3 h-11 w-auto max-w-full origin-left scale-[1.55] object-contain min-[361px]:scale-[1.63] sm:mt-4 sm:h-[52px] sm:scale-[1.7] lg:mt-5 lg:h-16 lg:scale-[1.75] xl:h-20 xl:scale-[1.8] 2xl:h-24 2xl:scale-[1.9]"
          />
        </Link>
      ...
      {menuOpen && (
        <div className="w-full max-w-full overflow-x-hidden max-h-[calc(100dvh-72px)] overflow-y-auto border-t border-gray-100 bg-white/95 px-4 py-4 shadow-2xl backdrop-blur-md animate-[mobileMenuSlide_0.28s_ease-out] sm:max-h-[calc(100dvh-95px)] lg:hidden">
